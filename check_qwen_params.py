...
model_path = './folds/3000/distilled_gemma2-9b_fold_0'  # Fold model has the trained 3-class head
...
test_cases = [
    ...
    },
    {
        ...
        'expected': 'A and B are close'
    },
    {
        'prompt': "How to initialize the classification head when I do transfer learning. For example, I have a pre-trained vision transformer on ImageNet, and now I want to finetune it on StanfordCars,I want to do full finetuning",
...
for test in test_cases:
    text = f"Question: {test['prompt']}\n\nAnswer A: {test['response_a']}\n\nAnswer B: {test['response_b']}"
    ...
    print(f"\nTest: {test['expected']}")
    print(f"  model_a: {probs[0]:.4f}")
    print(f"  model_b: {probs[1]:.4f}")
    print(f"  tie:     {probs[2]:.4f}")
    
    # Check if prediction matches expected result
    expected = test['expected']
    if 'B wins clearly' in expected:
        if probs[1] > 0.5:
            print(f"  ✅ PASS - B correctly predicted as winner")
        else:
            print(f"  ❌ FAIL - Expected B to win (>0.5), but got model_b={probs[1]:.4f}")
    elif 'A wins clearly' in expected:
        if probs[0] > 0.5:
            print(f"  ✅ PASS - A correctly predicted as winner")
        else:
            print(f"  ❌ FAIL - Expected A to win (>0.5), but got model_a={probs[0]:.4f}")
    elif 'close' in expected.lower() or 'tie' in expected.lower():
        if probs[2] > 0.5:  # Tie should have reasonable probability
            print(f"  ✅ PASS - Tie/close prediction is reasonable")
        else:
            print(f"  ⚠️  WARN - Expected tie/close, but tie={probs[2]:.4f} is low")
    else:
        print(f"  ⚠️  Unknown expected outcome")