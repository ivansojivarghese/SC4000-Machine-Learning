...
LLAMA_TOK=${LLAMA_TOK:-meta-llama/Meta-Llama-3.1-8B}
QWEN_TOK=${QWEN_TOK:-Qwen/Qwen2.5-14B}
GEMMA_TOK=${GEMMA_TOK:-google/gemma-2-9b}

LLAMA_BASE=${LLAMA_BASE:-meta-llama/Meta-Llama-3.1-8B}
QWEN_BASE=${QWEN_BASE:-Qwen/Qwen2.5-14B}
...
if [ "${AUTO_DOWNGRADE}" = "1" ] && [ "${GPU_MEM_GB}" -ne 0 ] && [ "${GPU_MEM_GB}" -le 32 ]; then
  ...
  if [[ "${LLAMA_BASE}" == *"70B"* || "${LLAMA_BASE}" == *"72B"* ]]; then
    ...
    LLAMA_TOK=meta-llama/Meta-Llama-3.1-8B
  fi
fi
